...
router.get("/search", (req, res) => {
  ...
  if (typeof star_rating == "undefined" || star_rating == "") star_rating = 0;
  if (typeof review_score == "undefined" || review_score == "")
    review_score = 0;
  if (typeof price_low == "undefined" || price_low == "") price_low = 0;
  ...
  Hotel.find({
    amenities: {
      $all: [free_wifi, pool, free_parking, multilingual, free_breakfast]
    },
    $and: [
      { "price.singlePrice": { $gte: price_low } },
      { "price.singlePrice": { $lte: price_high } }
    ],
    ...
    .then((doc, err) => {
      ...
      while (result.length < NUM_RESULTS && doc[startIndex] !== undefined) {
        ...
        let singleAvailable = checkAvailable(
          doc[startIndex].roomTypeAndNumber.single,
          date,
          numberRooms,
          bookingID
        );
        let doubleAvailable = checkAvailable(
          doc[startIndex].roomTypeAndNumber.double,
          date,
          numberRooms,
          bookingID
        );
        let kingAvailable = checkAvailable(
          doc[startIndex].roomTypeAndNumber.king,
          date,
          numberRooms,
          bookingID
        );
        let studioAvailable = checkAvailable(
          doc[startIndex].roomTypeAndNumber.studio,
          date,
          numberRooms,
          bookingID
        );
...
router.get("/individual", async (req, res) => {
  ...
  Hotel.findById(req.query.id)
    .then(hotel => {
      ...
      Booking.find({ hotelID: req.query.id })
        .then(function grabBookings(allBookings) {
          ...
          async function getReviews() {
            ...
            reviewPack = Promise.all(
              filteredBookings.map(async elem => {
                // Get the full name of the customer for this Booking
                let cust = await Customer.findById(elem.customerID).catch(
                  error => {
                ...
                let reviewItem = {
                  ...
                  reviewName: `${cust.Firstname} ${cust.Lastname.charAt(
                    0
                  ).toUpperCase()}.`
                };
          ...
          getReviews().then(async () => {
            if (
              typeof date.checkin !== "undefined" &&
              typeof date.checkout !== "undefined"
            ) {
              if (
                checkAvailability(
                  hotel.roomTypeAndNumber.single,
                  date,
                  numberOfRooms,
                  "PlaceHolder"
                ).length == 0
              ...
              if (
                checkAvailability(
                  hotel.roomTypeAndNumber.double,
                  date,
                  numberOfRooms,
                  "PlaceHolder"
                ).length == 0
              ...
              if (
                checkAvailability(
                  hotel.roomTypeAndNumber.studio,
                  date,
                  numberOfRooms,
                  "PlaceHolder"
                ).length == 0 ||
            ...
            individualHotelPack = {
              ...
              review: (await reviewPack).sort((a, b) => {
                return (
        ...
        .catch(err => {
          return res.status(404).json({
            err: `Couldn't find any bookings for hotel ${req.query.id}`
          });
        });
    })
    .catch(err => {
      return res
        .status(404)
        .json({ err: `Couldn't find a hotel for ${req.query.id}` });
    });
