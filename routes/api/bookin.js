...
router.post("/review", (req, res) => {
  ...
  Booking.findByIdAndUpdate(
    req.body.bookingID,
    { review: req.body.comment, starReview: req.body.starRating },
    (err, booking) => {
            ...
            if (historyPack.length === bookings.length) {
              historyPack.sort((a, b) => {
                if (b.new_check_in_date && a.new_check_in_date) {
                  return (
                    new Date(a.new_check_in_date).getTime() -
                    new Date(b.new_check_in_date).getTime()
                  );
                } else if (
                  b.new_check_in_date &&
                  a.new_check_in_date === undefined
                ) {
                  return (
                    new Date(a.new_check_in_date).getTime() -
...
router.post("/confirm", (req, res) => {
  passport.authenticate("jwt", function(err, user, info) {
    ...
    var rewardPointsUsed = req.body.rewardPointsUsed
      ? parseInt(req.body.rewardPointsUsed)
      : null;
    var rewardPointsEarned = req.body.rewardPointsEarned
      ? parseInt(req.body.rewardPointsEarned)
      : null;
    var rewardDiscount = req.body.rewardDiscount
      ? req.body.rewardDiscount
      : null;
        ...
        if (checkAvailability(arr, date, numberRooms, bookingID).length !== 0) {
          ...
          Booking.find({
            $and: [
              { customerID: customerID },
              {
                $or: [
                  ...
                  { new_check_in_date: date.checkin }
                ]
              }
            ]
          }).then((doc, err) => {
            ...
            // if the customer already have one reservation for the same checkin date, return error message
              ...
              res.send({ message: "doubleBooking", code: 403 });
            }
          });
        }
        // if the type of room is not avaliable
        else {
          res.send({ message: "noRoomAvailable", code: 409 });
        }
      });
    }
  })(req, res);
  ...
  passport.authenticate("jwt", function(err, user, info) {
    ...
    Booking.findById(bookingID).then((reservations, err) => {
      ...
      if (reservations) {
        ...
        date = {
          checkin: new Date(
            req.body.newCheckIn.replace('"', "").replace('"', "")
          ),
          checkout: new Date(
            req.body.newCheckOut.replace('"', "").replace('"', "")
          )
        };
          ...
          if (reservations) {
            ...
            if (reservations.changed) {
              if (
                ...
                res
                  .status(409)
                  .json({ message: "cannot change to same dates" });
                return;
              }
            }

            Hotel.findById(reservations.hotelID)
              .then(hotel => {
                ...
                if (
                  checkAvalibity(arr, date, reservations.numOfRoom, bookingID)
                ) {
                  if (isLogged) {
                    console.log("is here");
            ...
            res
              .status(404)
              .json({ message: `cannot find ${bookingID}`, code: 404 });
          }
        });
      }
    })(req, res);
...
router.post("/cancel", (req, res) => {
  Booking.findById(req.body.bookingID).then(booking => {
    if (booking.status === 0) {
      booking.status = 3;
      if (booking.changed) {
        if (booking.new_check_in_date - new Date() > 172800000) {
          if (booking.rewardPointsUsed !== 0) {
            User.find({ customerID: booking.customerID }).then(user => {
      ...
      Hotel.findById(booking.hotelID).then(hotel => {
