  ...
  Recipe.findById(req.params.id)
    .then(recipe => {
      if (recipe) {
        ...
        return res.status(404).json({ norecipefound: 'Such recipe does not exist' });
      }
    })
  ...
  Profile.findOne({ user: req.user.id })
    .then(profile => {
      Recipe.findById(req.params.id)
        ...
        .catch(err => res.status(404).json({ norecipefound: "Recipe doesn't exist" }));
    })
    .catch(err => res.status(401).json({ notauthorized: "User is not logged in" }));
});
